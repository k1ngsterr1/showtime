    ...
    builder
      ...
      .addCase(detectFaces.fulfilled, (state, action) => {
        ...
        if (action.payload.length === 0) {
          msg = [
            "The face could not be identified. Insert an IMAGE where the face is clear.",
          ];
        } else if (
          action.payload.length === 1 &&
          action.payload[0].detection.score <= 0.75
        ) {
          msg = [
            "You got a score of " +
              Math.round(action.payload[0].detection.score * 100) / 100 +
              " which is below the minimum threshold of 0.75. Insert an IMAGE where the face is more visible.",
          ];
        } else if (action.payload.length > 1) {
          msg = [
            "More than one face has been detected. Insert an IMAGE where there is only you.",
          ];
        }
        state.error = msg;
        ...
        state.styles.preview =
          msg != null
            ? { ...state.styles.preview, borderColor: "#f00000" }
            : { ...state.styles.preview, borderColor: "#00ff00" };
      })
      .addCase(detectFaces.rejected, (state) => {
        state.status = "rejected";
        state.error = [
          "A general error occurred with face detection. Please, try again.",
        ];
        state.styles.preview = {
          ...state.styles.preview,
          borderColor: "#f00000",
...
export const { setStyles, setFacenetError, setFacenetMessage, setOutline } =
  facenetSlice.actions;
