...
export const setScreenshotFromURL = createAsyncThunk(
  ...
  async (credentials, { rejectWithValue }) => {
    let response = {};
    await axios
      .post(GET_IMAGE_URL, { ...credentials })
      .then((r) => {
        response = { status: r.status, data: r.data };
      })
      .catch((e) => {
    ...
    builder
      ...
      .addCase(registerUser.rejected, (state, action) => {
        ...
        if (action.payload.response) {
          ...
          state.error.register.serverErr =
            "SERVER ERROR: " + action.payload.response.data;
        } else {
          state.request.code = 500;
          state.error.register.serverErr =
            "SERVER ERROR: " + action.payload.message;
        }
      })
